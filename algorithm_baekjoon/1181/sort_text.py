...
풀이
sort, sorted의 차이점과 정렬조건을 알아야 한다.
```
sort = 원본 리스트를 정렬
...
```
입력을 받는 과정에서 공백 제거함수```strip```를 사용한 이유는 아래 lambda식을 사용하면
정렬 시, 공백을 포함하여 리스트에 추가된다.   
...
해당 조건에서 길이 순서대로 오름차순 정렬하고, 이후 같으면 사전 순으로 정렬하도록 조건을 추가하였다.
