...
두번째 줄부터 수행 과정을 출력한다. 두번재 줄부터 K개의 줄에 걸쳐 두 정수 A B를 빈 칸을
사이에 두고 출력하는데, A번째 탑의 가장 위에 있는 원판을 B번째 탑의 가장 위로 옮긴다는 뜻이다.

풀이
하노이 탑의 이동 순서는 n개의 원판이 있을 때, n-1개의 원판들을 2번으로 옮기고
마지막 남은 하나의 원판을 1번 -> 3번으로 옮긴다. 그리고 다시 n-1개의 원판을 2 -> 3번으로 옮긴다.
이러한 규칙에 의해 하노이탑이 옮겨지게 된다.
...
이동 횟수 K는 2^n - 1인데 해당 시행 횟수가 이렇게 되는 이유는
