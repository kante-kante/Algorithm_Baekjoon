...
풀이:
N*M 보드에서 N은 행, M은 각 요소의 갯수(W,B의 갯수)를 의미한다.
이후 행렬, 배열의 형태로 나타내기 위해 리스트를 하나 선언해주고
반복문을 이용하여 원래 체스판 행의 갯수를 미리 지정해준다(N)

이후 결과 배열을 표시하기 위해 리스트를 다시 하나 선언해준다.
체스판에서 첫 시작점인 첫번째 행을 지정해주기 위하여 다음과 같이 N-7로 인덱스를 고정시킨다.
첫 시작점을 지정한 다음 첫 칸(열)을 지정하기 위해 M도 인덱스를 고정시켜준다.
