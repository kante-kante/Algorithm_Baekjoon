...
풀이:
식의 결과를 최소값이 되도록 만드는 수. 따라서 '-'를 만나기 전까지 모든 숫자는 더해주고
'-'를 만나면 -의 뒤부터 ()를 쳐서 뒤의 수들을 빼주는 형식으로 계산한다.
...
두번째 반복문은 1번째 원소, 즉, 두번째 값부터는 모두 빼주어야 할 숫자들이다.
왜냐하면 이미 입력값에서 -를 기준으로 값을 split해두었기 때문에
